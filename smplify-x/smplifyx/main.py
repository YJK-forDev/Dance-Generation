...
def main(**args):
    ...
    for idx, data in enumerate(dataset_obj):
        ...
        curr_camera_folder = osp.join(output_folder, "camera_params", fn)
        if not osp.exists(curr_camera_folder):
            os.makedirs(curr_camera_folder)
        curr_smplx_mesh = osp.join(output_folder, "smplx_mesh", fn)
        if not osp.exists(curr_smplx_mesh):
            os.makedirs(curr_smplx_mesh)
        ...
        for person_id in range(keypoints.shape[0]):
            ...
            curr_result_fin = osp.join(curr_result_folder,
                                        '{:03d}_final.pkl'.format(person_id))
            ...
            fit_single_frame(img, keypoints[[person_id]],
                                ...
                                result_fin=curr_result_fin,
                                mesh_fn=curr_mesh_fn,
                                smplx_fn = curr_smplx_mesh,
                                camera_fn=curr_camera_folder,
