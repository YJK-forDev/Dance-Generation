...
class SMPLPixDataset(Dataset):
    ...
    def __init__(self, data_dir,
        ...
        self.camera_dir = os.path.join(data_dir, 'camera_params')
    ...
    def __getitem__(self, idx):
        ...
        image_id = "{:03d}".format(0)
        person_id = 0

        if os.path.exists(self.camera_dir):
            with open(os.path.join(self.camera_dir, img_name.split('.')[0], "000.pkl"), "rb") as fr:
                data = pickle.load(fr)
                camera_rotation = data['camera_transl']

            if camera_rotation[0] < 0 and person_id == 0:
                image_id = "{:03d}".format(0)
            elif camera_rotation[0] > 0 and person_id == 1:
                image_id = "{:03d}".format(0)
            else:
                image_id = "{:03d}".format(1)

        if image_id is not None:
            x_path = os.path.join(self.input_dir, img_name.split('.')[0], image_id +'.png')
        else:
